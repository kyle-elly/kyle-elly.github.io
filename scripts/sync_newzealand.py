#!/usr/bin/env python3
"""Download new newzealand photos from Drive, generate 600px grid thumbnails,
   and update newzealand_manifest.json.
   Idempotent: only new files are processed.

   Structurally identical to sync_photographer.py / sync_booth.py / sync_gallery.py
   but reads from a different Drive folder (via NEWZEALAND_DRIVE_FOLDER_ID) and
   writes to newzealand_thumbnails/ and newzealand_manifest.json so the pipelines
   never touch each other's files.

   Same two intentional DIFFs as the photographer set, because the newzealand
...
FOLDER_ID = os.environ["NEWZEALAND_DRIVE_FOLDER_ID"]   # newzealand folder ID, injected by workflow
SA_FILE   = os.environ["NEWZEALAND_DRIVE_SA_FILE"]

THUMB_DIR = Path("newzealand_thumbnails")
MANIFEST  = Path("newzealand_manifest.json")
...
MAX_PER_RUN = int(os.environ.get("NEWZEALAND_MAX_PER_RUN", "0"))
...
def location_of(f):
    """Return (latitude, longitude) from Drive image metadata.
    Missing coordinates and the no-fix sentinel (0, 0) become (None, None).
    """
    meta = f.get("imageMediaMetadata") or {}
...
def main() -> int:
    ...
    print(f"Drive returned {len(drive_files)} newzealand images; "
    ...
    for f in drive_files:
        ...
        manifest[fid] = {
            ...
            "w": w,
            "h": h,
            "caption": "",          # ← DIFF: newzealand photos carry no caption
        }
        added += 1
    ...
    print(f"Done. Added {added} new newzealand files. Total: {len(manifest)}.")
    return 0
